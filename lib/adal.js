...
AuthenticationContext.prototype._renewToken = function(resource, callback) {
    ...
    this.resourceCallbacks = this.resourceCallbacks || {};
    this.resourceCallbacks[expectedState] = callback;
    this.idTokenNonce = null;
    this._logstatus('Navigate to:' + urlNavigate);
    this._saveItem(this.CONSTANTS.STORAGE.LOGIN_REQUEST, '');

    if (window.cordova) {
        this.hiddenWindowFetch(urlNavigate, callback);
    }
    else {
        var frameHandle = this._addAdalFrame('adalRenewFrame' + expectedState);
        frameHandle.src = 'about:blank';
        this._loadFrame(urlNavigate, 'adalRenewFrame' + expectedState);
    }
};
...
AuthenticationContext.prototype.hiddenWindowFetch = function(urlNavigate, callback) {
    this._logstatus('Add hidden window for executing ' + urlNavigate);
    var self = this;
    var frame = window.open(urlNavigate, '_blank', 'hidden=yes,location=yes');

    var handleEvent = function(event) {
        console.log("loadstart " + event.url);
        var match = event.url.match(/.*\#(.*)/);
        if (!match) {
            return;
        }
        var hash = match[1];
        var isCallback = self.isCallback(hash);
        if (isCallback) {
            console.log("Is Callback. Doing the thing.");
            var hash = self._getHash(hash);
            console.log(hash);
            var requestInfo = self.getRequestInfo(hash);
            self.saveTokenFromHash(requestInfo);

            console.log(requestInfo);
            console.log("Hidden frame fetched it");
            frame.executeScript({
                code: 'window.location = "about:blank"; window.close();'
            }, function() {
                frame.close();
            });
            
            if (self.resourceCallbacks[requestInfo.stateResponse]) {
                var callback = self.resourceCallbacks[requestInfo.stateResponse];
                self.resourceCallbacks[requestInfo.stateResponse] = undefined;
            }
            ...
            callback(self._getItem(self.CONSTANTS.STORAGE.ERROR_DESCRIPTION), requestInfo.parameters[self.CONSTANTS.ACCESS_TOKEN]);
        }
    };

    var handleExit = function() {
        frame.removeEventListener('loadstart', handleEvent);
        frame.removeEventListener('exit', handleExit);
    };
    frame.addEventListener('loadstart', handleEvent);
    frame.addEventListener('exit', handleExit);
};
